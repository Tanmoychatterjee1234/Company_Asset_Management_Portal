...
app.post('/loginUser', async (req, res) => {
    ...
    try {
        ...
        if (result.length > 0) {
            ...
            const updateQuery = `Update Users set sessionStatus = 'Active' WHERE employeeId = ?`;
            await pool.execute(updateQuery, [employeeId]);
...
app.get('/sessionData', async (req, res) => {
    ...
    if (req.session.user) {
        const query = `Select sessionStatus from users where employeeId = ?`;
        const currentSessionStatus = await pool.execute(query, [req.session.user.employeeId]);
        res.json({
            ...
            sessionStatus: currentSessionStatus[0][0].sessionStatus
        });
    } else {
...
app.get('/logout', async (req, res) => {
    if (req.session.user) {
        const updateQuery = `Update Users set sessionStatus = 'Inactive' WHERE employeeId = ?`;
...
app.post('/assets/search', async (req, res) => {
    ...
    const query = `SELECT * FROM Assets WHERE ${field} = ?`;
...
app.put('/assets/update/:_assetSerialNumber/:subLocation/:userType', async (req, res) => {
    ...
    if (checkResult["subLocation"] != subLocation && userType == 'user') {
...
app.get('/assets/downloadAssets/:subLocation', async (req, res) => {
    ...
    try {
        ...
        if (subLocation != 'All Data') {
            const checkQuery = `SELECT * FROM assets WHERE subLocation = ?`;
...
app.put('/assets/delete/:assetSerialNumber', async (req, res) => {
    ...
    const query = `
        UPDATE assets SET status='Scrapped' , transactionDate = ?
        WHERE assetSerialNumber = ? 
    `;
